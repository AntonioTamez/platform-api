...
- **Global scope**: `C:/ATS/Git/platform/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/ATS/Git/platform/.claude/notes/`)
...
2. Glob `C:/ATS/Git/platform/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/ATS/Git/platform/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/ATS/Git/platform/.claude/notes/` — works in any directory
